...
p.add_argument('--experiment_name', default="new", type=str,
...
for png_file in jpg_files[:10]:
    ...
    coord_dataset = dataio.Implicit2DWrapper(img_dataset, sidelength=32, compute_diff='none', grid='our')
    ...
    coord_dataset_siren = dataio.Implicit2DWrapper(img_dataset_siren, sidelength=32, compute_diff='none', grid='siren')
    image_resolution = (32, 32)

    dataloader_siren = DataLoader(coord_dataset_siren, shuffle=True, batch_size=opt.batch_size, pin_memory=True,
                                  num_workers=0)
    ...
    psnr_ours = training.train(model=model_ours, train_dataloader=dataloader, epochs=opt.num_epochs_ours, lr_init=1e-3,
                               lr_finish=1e-4,
