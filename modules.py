...
class ImplicitMLPShort(nn.Module):
    ...
    def forward(self, model_input):
        ...
        coords = coords_org# torch.Size([1, 2, 32, 32])

        x = rearrange(coords, "b c h w -> b (h w) c")  # Flatten the images
        x = self.linear1(x)
        x = F.relu(x)
        x = self.linear2(x)
        x = F.relu(x)
...
class ImplicitMLP(nn.Module):
    def __init__(self, B):
        ...
        self.gff = GaussianFourierFeatureTransform(mapping_dim=64, B=B)
        self.linear1 = nn.Linear(128, 64)  # FMMLinear(256, 256, 100)
